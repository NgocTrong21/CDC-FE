...
const UpdatePermission = () => {
      ...
      .then((res: any) => {
        ...
        let array: any = a.map((item: any) => {
          item?.permissions?.forEach((x: any) => {
            let check_array = b?.Role_Permissions?.filter((y: any) => x.id === y.permission_id);
            x.is_check = check_array?.length === 0 ? false : true;
          });
          let isFullCheck: any = item?.permissions?.filter((x: any) => x.is_check === false);
  ...
  const setCheckRole = (item: any) => {
    let r: any = permissions.map((permission: any) => {
      if(item.id === permission.id) {
        ...
        let i = permission.permissions.map((x: any) => ({ ...x, is_check: !x.is_check}));
  ...
  const setCheckPermission = (item: any) => {
    ...
    let r: any = permissions.map((permission: any) => {
      let isFullCheck: any = permission.permissions.filter((y: any) => y.is_check === false);
  ...
  const updatePermission = () => {
    ...
    permissionApi.updatePermissionForRole({ role_id: id, permissions: r })
      .then((res: any) => {
            ...
            permissions.map((item: any) => (
              <div className='cursor-pointer'>
                <div className='flex flex-row gap-4' key={item?.id}>
                  <Checkbox checked={item?.is_check} onChange={() => setCheckRole(item)}/>
                  <div className='text-base font-bold mb-2'>{item?.display_name}</div>
                  <div className='hover:transition-all' onClick={() => setActive(item)}>
                    {
                      !item?.is_active ? <RightOutlined /> : <DownOutlined />
                    }
                  </div>
                </div>
                <div className={`ml-8 transition-all ${item?.is_active ? 'block' : 'hidden'}`}>
                  {
                    item?.permissions?.length > 0 && item?.permissions?.map((permission: any) => (
                      <>
                        <div className='flex flex-row gap-4' key={permission?.id}>
                          <Checkbox checked={permission?.is_check} onChange={() => setCheckPermission(permission)} />
                          <div className='text-base font-bold mb-2'>{permission?.display_name}</div>
                        </div>
                      </>
                    ))
                  }
