...
import { Checkbox, Divider, Input, Pagination, Row, Select, Table, Tabs } from 'antd'
...
import { getDataExcel, getFields, options } from 'utils/globalFunc.util';
...
const Statistic = ({ param_string, selects, title, show_tab, onChangeTab }: any) => {
  ...
  const search = (name: string, param: any, page: number) => {
  ...
  const downloadEquipmentList = () => {
    ...
    const fileName = `Danh sách thiết bị ${new Date().toISOString().substring(0, 10)}.xlsx`;
    ...
    <div>
      <div className="flex-between-center">
        ...
        <div className='flex flex-row gap-6'>
          <ExportToExcel getData={downloadEquipmentList} />
        </div>
      </div>
      <Divider />
