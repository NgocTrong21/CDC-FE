...
exports.updateGroup = async (req, res) => {
  try {
    await db.sequelize.transaction(async (t) => {
      ...
      if (isDuplicateGroup) return errorHandler(res, err.EQUIPMENT_GROUP_DUPLICATED);
      await db.Equipment_Group.update(
        req.body,
        {
          where: { id: req?.body?.id },
          transaction: t
...
exports.updateType = async (req, res) => {
  try {
    await db.sequelize.transaction(async (t) => {
      ...
      if (isDuplicateType) return errorHandler(res, err.EQUIPMENT_TYPE_DUPLICATED);
      await db.Equipment_Type.update(
        req.body,
        {
          where: { id: req?.body?.id },
          transaction: t
...
exports.updateUnit = async (req, res) => {
  try {
    await db.sequelize.transaction(async (t) => {
      ...
      if (isDuplicateUnit) return errorHandler(res, err.EQUIPMENT_UNIT_DUPLICATED);
...
exports.createStatus = async (req, res) => {
  try {
    await db.sequelize.transaction(async (t) => {
      ...
      if (isHasStatus) return errorHandler(res, err.EQUIPMENT_STATUS_DUPLICATED);
...
exports.updateStatus = async (req, res) => {
  try {
    await db.sequelize.transaction(async (t) => {
      ...
      if (!isHasStatus) return errorHandler(res, err.EQUIPMENT_STATUS_NOT_FOUND);
      let isDuplicateStatus = await db.Equipment_Status.findOne({
        where: { name: req?.body?.name }
      });
      if (isDuplicateStatus) return errorHandler(res, err.EQUIPMENT_STATUS_DUPLICATED);
      await db.Equipment_Status.update(
        req.body,
        {
          where: { id: req?.body?.id },
          transaction: t
...
exports.updateRiskLevel = async (req, res) => {
  try {
    await db.sequelize.transaction(async (t) => {
      ...
      if (isDuplicateRiskLevel) return errorHandler(res, err.RISK_LEVEL_DUPLICATED);
      await db.Equipment_Risk_Level.update(
        req.body,
        {
          where: { id: req?.body?.id },
          transaction: t
